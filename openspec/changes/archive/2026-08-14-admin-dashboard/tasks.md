...
- [x] 4.1 Add `backend/vercel.json`, `frontend/vercel.json`, and deployment documentation for separate roots/environments; smoke backend before frontend and DNS.
- [x] 4.2 Document independent deployment rollback and restoring the prior passing SHA/generated validators; the provider-before-consumer CI requirement is already enforced in PR 1 task 1.6.

### PR 4 Boundary (`deployment-verification`: tasks 4.1–4.2)

- [x] Focused test: no runtime code changed (documentation/deployment-config only), so no package test suite applies. Verified instead: `node -e "JSON.parse(...)"` on both new `vercel.json` files (valid JSON) and full-workspace `pnpm typecheck` (3/3 packages) and `pnpm lint` (3/3 packages) both exit 0, confirming Phase 4 introduced no regression in `contracts`/`backend`/`frontend`.
- [x] Runtime harness: N/A — there is no live Vercel account in this environment to actually deploy to, so the two real deployment/DNS-cutover runtime boundaries this phase documents (backend-then-frontend live smoke test, `admin.cargable.es` DNS cutover) cannot be exercised here. Substituted the closest available proof: ran `pnpm --filter @cargable/backend build` (`tsc --noEmit`, exit 0) and `pnpm --filter @cargable/frontend build` (`tsc --noEmit`, exit 0) to confirm each app still builds standalone, then additionally ran `pnpm --filter @cargable/frontend exec next build` with placeholder env vars — the actual command `frontend/vercel.json`'s `buildCommand` invokes — which succeeded standalone and produced the expected route manifest (`/login`, `/business`, `/invoices`, `/operations`, `/api/dashboard/[...path]`, `Proxy (Middleware)`). This last check also surfaced a real discrepancy (documented in `docs/deployment.md` and not silently fixed): every workspace package's `"build"` script is `tsc --noEmit`, which is correct for `backend`/`packages/contracts` but insufficient for `frontend`, so `frontend/vercel.json` overrides `buildCommand` to call `next build` directly instead of relying on the package's own `build` script.
- [x] Limit: authored footprint **~176 lines** — `backend/vercel.json` (4 lines), `frontend/vercel.json` (4 lines), `docs/deployment.md` (168 lines). Well within the single-PR 400-line cap; no exception needed.
- [x] Rollback: revert `backend/vercel.json`, `frontend/vercel.json`, and `docs/deployment.md` as one unit; no application code, tests, or contract files are touched.
- [x] Out of scope: `backend/src/`, `frontend/src/`, `frontend/app/`, `packages/contracts/`, database schema/migrations, RPC implementation, real customer data, and actual Vercel project creation/deployment/DNS changes (no live account in this environment) — confirmed untouched.
