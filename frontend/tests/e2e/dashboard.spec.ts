...
test("an unauthenticated visitor is redirected away from a protected report without seeing report data", async ({
  ...
  await expect(page.getByText(/usuarios registrados/i)).toHaveCount(0);
});
...
test.describe("authorized administrator session", () => {
  ...
  test("selecting a range shows the requested window or a labelled snapshot", async ({ page }) => {
    ...
    await expect(page.getByText(/Rango 7d:|Instantánea/)).toBeVisible();

    await page.getByRole("button", { name: "Todo el tiempo" }).click();
    await expect(page.getByText("Instantánea — no hay un rango de tiempo confiable disponible")).toBeVisible();
  });
  ...
  test("an empty report shows an empty state rather than an error", async ({ page }) => {
    ...
    const empty = page.getByText(/No hay datos de facturas/);
  ...
  test("a refresh failure retains the last known data and marks it stale", async ({ page, context }) => {
    ...
    await expect(page.getByText(/Desactualizado — mostrando los últimos datos conocidos/)).toBeVisible();
    await expect(page.getByText("Usuarios registrados")).toBeVisible();
  });
  ...
  test("the reporting interface never exposes mutation controls or raw data", async ({ page }) => {
    ...
    for (const path of ["/business", "/invoices", "/operations"]) {
      ...
      await expect(page.locator("form:not([aria-label='Inicio de sesión de administrador'])")).toHaveCount(0);
